...
# List comprehension
# [mapeamento(x) for x : colecao if filtro(x)]
# Parte 1: mapeamento(x) ou somente X, é quem vai ser adicionado na lista
# Parte 2: For x : colecao, é a iteração da lista
# Parte 3: filtro(x), é opicional, e se estiver presente vai determinar quem vai para a PARTE 1

lista = ['a', 'b', 'c']
lista_ABC = [item.upper() for item in lista]
print(lista_ABC)
lista_TAM = [len(item) for item in lista]
print(lista_TAM)
